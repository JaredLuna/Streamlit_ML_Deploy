...
modelo_recomendation = joblib.load('..\\Model_Recomendation.joblib')
# Cargar el dataset
df_recomendation = pd.read_parquet('..\\Data\\CaliforniaRevs.parquet', engine= 'pyarrow')
...
data_recomendation = Dataset.load_from_df(df_recomendation[['user_id', 'restaurant_id', 'rating']], reader_recomendation)
...
df_recomendacion = df_clean.head(50000)
...
model_choice = st.selectbox("Elige el modelo:", ("Regresión Logística", "Random Forest")) #Aquí solo hay que dejar la opción logistica nada más
...
if st.button("Obtener recomendaciones"):
    ...
    for restaurant_id, estimated_rating in recommendations:
        st.write(f"Restaurant: {restaurant_id}, Calificación estimada: {estimated_rating}")
...
if st.button("Analizar Aspectos"):
    ...
    st.write(f"Servicio: {'Positivo' if aspects[0] else 'Negativo'}")
    st.write(f"Comida: {'Positivo' if aspects[1] else 'Negativo'}")
    st.write(f"Ambiente: {'Positivo' if aspects[2] else 'Negativo'}")